...
void make_sym(int idx)
{
	...
	if (idx == 3)
	{
		...
		temp[1] = _cal3(arr[0], arr[1], _cal2(arr[2], arr[3], sym[2]), sym[0], sym[1]);
		temp[2] = _cal2(_cal2(arr[0], arr[1], sym[0]), _cal2(arr[2], arr[3], sym[2]), sym[1]);
		temp[3] = _cal2(_cal3(arr[0], arr[1], arr[2], sym[0], sym[1]), arr[3], sym[2]);
		temp[4] = _cal2(arr[0], _cal3(arr[1], arr[2], arr[3], sym[1], sym[2]), sym[0]);
		temp[5] = _cal4();
		...
		chk(temp);
		//if (temppp != -23) 
			//cout << arr[0] << sym[0] << arr[1] << sym[1] << arr[2] << sym[2] << arr[3] << " = " << arr[4] << " // " << temppp << endl;
		return;
	}
	for (int i = 0; i < 4; i++)
	{
